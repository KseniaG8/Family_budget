...
class TransactionHandler {
  public:
    ...
    nlohmann::json setLimit(int user_id, const std::string &category, double limit_amount, const std::string &period);
    nlohmann::json checkLimit(int user_id, const std::string &category, const std::string &period);

    nlohmann::json addGoal(int user_id, const std::string &name, double target_amount, const std::string &deadline);
    nlohmann::json getGoals(int user_id);
    nlohmann::json updateGoalProgress(int goal_id, double current_amount);
    nlohmann::json updateGoal(int goal_id, const std::string &name, double target_amount, const std::string &deadline);
    nlohmann::json deleteGoal(int goal_id);

    nlohmann::json createGroup(const std::string &name, int owner_id);
    nlohmann::json deleteGroup(int group_id);
    nlohmann::json addUserToGroup(int group_id, int user_id);
    nlohmann::json removeUserFromGroup(int group_id, int user_id);
    nlohmann::json getGroupMembers(int group_id);
    ...
    nlohmann::json getGroupTransactions(int group_id);
    nlohmann::json getGroupBalance(int group_id);
