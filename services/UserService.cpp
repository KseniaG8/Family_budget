...
nlohmann::json UserService::loginUser(const std::string& login, const std::string& password) {
    ...
    if (user.id == -1) return {{"status", "error"}, {"message", "Неверные данные"}};
    if (!Botan::check_bcrypt(password, user.password)) return {{"status", "error"}, {"message", "Неверные данные"}};

    return {{"status", "success"}, {"is_2fa_enabled", user.is_2fa_enabled == 1}, {"user_id", user.id}, {"login", user.login}}; 
}
...
nlohmann::json UserService::setup2FA(int user_id, const std::string& login) {
    ...
    display_secret.erase(std::remove(display_secret.begin(), display_secret.end(), '='), display_secret.end());
    ...
    if (database.enable2FA(user_id, db_secret)) {
        ...
        return {{"status", "success"}, {"secret", display_secret}, {"uri", uri}};
    }
    
    return {{"status", "error"}, {"message", "Database error"}};
}

nlohmann::json UserService::verifyLogin2FA(const std::string& login, const std::string& code) {
    User user = database.getUserByLogin(login);
    ...
    try {
        ...
        if (totp.verify_totp(std::stoul(code), std::time(nullptr), 1)) {
            return {{"status", "success"}, {"user_id", user.id}, {"login", user.login}};
        }
    } catch (...) {}
